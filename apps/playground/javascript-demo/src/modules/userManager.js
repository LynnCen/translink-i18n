...
export function createUserManager(i18n) {
  ...
  return {
    ...
    listUsers() {
      ...
      for (const user of users.values()) {
        const status = user.isActive
          ? i18n.t('user.status.active')
          : i18n.t('user.status.inactive');
    ...
    searchUsers(query) {
      ...
      console.log(
        i18n.t('user.search.results', { query, count: results.length })
      );
    ...
    getStatistics() {
      ...
      const loggedIn = Array.from(users.values()).filter(
        u => u.lastLogin
      ).length;
      ...
      console.log(
        `${i18n.t('user.statistics.neverLoggedIn')}: ${stats.neverLoggedIn}`
      );
    ...
    validateUserData(data, isUpdate = false) {
      ...
      if (!isUpdate || data.email !== undefined) {
        if (!data.email || data.email.trim().length === 0) {
          errors.push(
            i18n.t('validation.required', { field: i18n.t('user.email') })
          );
      ...
      if (!isUpdate || data.password !== undefined) {
        if (!data.password || data.password.length === 0) {
          errors.push(
            i18n.t('validation.required', { field: i18n.t('user.password') })
          );
