...
test.describe('Vue Demo E2E Tests', () => {
  ...
  test.describe('基础功能', () => {
    test('应该显示中文欢迎消息', async ({ page }) => {
      ...
      await expect(page.locator('nav .nav-brand h1')).toContainText('TransLink I18n 演示');
    });
  ...
  test.describe('语言切换', () => {
    test('应该成功切换到英文', async ({ page }) => {
      ...
      await expect(page.locator('nav .nav-brand h1')).toContainText('TransLink I18n Demo');
      
      // 验证其他文本也更新了
      await expect(page.locator('.feature-grid .feature-card').first()).toContainText('Smart Code Transform');
    });
    ...
    test('应该成功切换到日文', async ({ page }) => {
      ...
      // 验证语言切换
      await expect(page.locator('h1')).toContainText('TransLink I18n へようこそ');
      
      // 验证功能卡片文本
      await expect(page.locator('.feature-grid .feature-card').first()).toContainText('スマートコード変換');
    });
  ...
  test.describe('交互功能', () => {
    test('应该处理用户信息表单', async ({ page }) => {
      const userInfoCard = page.locator('.demo-card').filter({ hasText: '用户信息' });
      
      // 检查用户信息卡片内容
      await expect(userInfoCard.locator('input[placeholder*="姓名"]')).toBeVisible();
      await expect(userInfoCard.locator('input[placeholder*="邮箱"]')).toBeVisible();
      
      // 填写表单
      await userInfoCard.locator('input[placeholder*="姓名"]').fill('张三');
      await userInfoCard.locator('input[placeholder*="邮箱"]').fill('zhangsan@example.com');
    ...
    test('应该处理联系表单', async ({ page }) => {
      const contactCard = page.locator('.demo-card').filter({ hasText: '表单示例' });
      
      // 填写联系表单
      await contactCard.locator('input[placeholder*="主题"]').fill('测试主题');
      await contactCard.locator('textarea[placeholder*="消息"]').fill('这是一条测试消息');
    ...
    test('应该显示数据统计', async ({ page }) => {
      const dataCard = page.locator('.demo-card').filter({ hasText: '数据展示' });
      
      // 检查数据展示元素
      await expect(dataCard.locator('.stat-item')).toHaveCount(3);
      await expect(dataCard.locator('.stat-item').first()).toContainText('总用户数');
      await expect(dataCard.locator('.stat-item').nth(1)).toContainText('活跃用户');
      await expect(dataCard.locator('.stat-item').nth(2)).toContainText('新增用户');
    ...
    test('应该处理通知演示', async ({ page }) => {
      const notificationCard = page.locator('.demo-card').filter({ hasText: '消息通知' });
      
      // 点击不同类型的通知按钮
      await notificationCard.locator('button').filter({ hasText: '成功' }).click();
      await expect(page.locator('.notification.success')).toContainText('操作成功');
      
      await notificationCard.locator('button').filter({ hasText: '警告' }).click();
      await expect(page.locator('.notification.warning')).toContainText('警告信息');
      
      await notificationCard.locator('button').filter({ hasText: '错误' }).click();
      await expect(page.locator('.notification.error')).toContainText('操作失败');
    });
  });
  ...
  test.describe('可访问性', () => {
    ...
    test('应该有正确的 ARIA 标签', async ({ page }) => {
      ...
      await expect(languageSelect).toHaveAttribute('aria-label', /语言选择|Language/);
